...
class RentalService:
    ...
    def remove_rental(self, item_identifier):
        confirmation = input(f"Are you sure you want to remove rental item '{item_identifier}'? (yes/no): ").lower()
    ...
    def search_by_keyword(self, keyword):
        found_items = [item for item in self.goods if
                       keyword.lower() in item.details.lower() or keyword.lower() in item.name.lower()]
        if found_items:
